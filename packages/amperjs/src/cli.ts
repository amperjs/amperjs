...
import amperFactory from "./factory";
...
if (viteBaseExists && tsconfigExists && packageJson?.distDir) {
  const { createSourceFolder } = await amperFactory(resolve(cwd, ".."), {
    NODE_VERSION,
  });

  // Current directory appears to be a valid AmperJS app,
  ...
  const { createApp } = await amperFactory(cwd, {
    NODE_VERSION,
  });

  // Prompting user to create a new AmperJS app...
  ...
  for (const line of [
    "",
    "🎉  Well Done! Your new AmperJS app is ready.",
    `─────────────────────────────────────────────`,
    "",

    `📂 Next steps:`,
    `- Navigate to your app dir - ${styleText("blue", `cd ./${app.name}`)}`,
